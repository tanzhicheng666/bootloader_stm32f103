...
typedef struct boot_info_t{
	...
	//uint32_t crc;//整体crc校验：改成双分区，去掉这个字段
	uint32_t firmware_size;//存放pending/testing状态固件的大小，用于CRC校验
...
void Bootloader_SetPending(uint32_t pending_app,uint32_t crc,uint32_t firmware_size);
